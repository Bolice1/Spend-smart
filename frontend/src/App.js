...
// ─── Log Expense ──────────────────────────────────────────────────────────────
function LogExpense({ expenses, setExpenses, currency, token }) {
  ...
  const submit = async () => {
    ...
    try {
      const result = await apiRequest("/api/v1/expenses", {
        method: "POST",
        token,
        body: {
          title: desc || CATEGORIES.find(c => c.id === cat)?.label || "Expense",
          description: desc,
          category: cat,
          amount: +amount,
          date,
      ...
      setExpenses(prev => [normalizeExpense(result.data), ...prev]);
  ...
  const remove = async (id) => {
    try {
      await apiRequest(`/api/v1/expenses/${id}`, { method: "DELETE", token });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (mode === "signup" && !result.data?.token) {
        const login = await apiRequest("/api/v1/auth/sign-in", { method: "POST", body: { email, password } });
        onAuth(login.data);
      } else {
        onAuth(result.data);
      }
      ...
      <form onSubmit={submit} style={{ ...S.panel, width: "100%", maxWidth: 420 }}>
        ...
        {mode === "signup" && (
          <div style={{ marginBottom: 14 }}>
            ...
            <input style={S.input} value={name} onChange={e => setName(e.target.value)} required minLength={6} />
          </div>
        )}
        <div style={{ marginBottom: 14 }}>
        ...
        <button
          type="button"
          style={{ ...S.btn("secondary"), marginTop: 12, border: "1.5px solid #e0e0ee" }}
...
export default function App() {
  ...
  const [auth, setAuth] = useStorage("ss_auth", null);
  ...
  useEffect(() => {
    ...
    setDisplayName(auth.user?.name || "User");
    setLoadingExpenses(true);
    setLoadError("");
    apiRequest("/api/v1/expenses", { token: auth.token })
      .then(result => setExpenses((result.data || []).map(normalizeExpense)))
  ...
  if (!auth?.token) return <AuthScreen onAuth={handleAuth} />;
  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.main}>
        ...
        <div style={S.content}>
          {loadError && <div style={{ ...S.panel, marginBottom: 20, color: "#EF4444" }}>{loadError}</div>}
          {loadingExpenses && <div style={{ ...S.panel, marginBottom: 20 }}>Loading expenses...</div>}
          {page === "dashboard" && <Dashboard expenses={expenses} budgets={budgets} currency={currency} navigate={setPage} />}
          {page === "log" && <LogExpense expenses={expenses} setExpenses={setExpenses} currency={currency} token={auth.token} />}
